...
import os
import argparse
from plotnine import ggplot, geom_boxplot, aes, position_dodge
######
# debbug
...
#                    then, OC_extention=task-HABLA1200_masked_epi_gm_ocDenoised_tsnr.nii.gz""")
###### Find files ###################################################################
df_oc=pd.DataFrame(columns=["value","pct_excluded","subj","method"])
df_tsnr=pd.DataFrame(columns=["value","subj","method"])
df_tsnr_prop=pd.DataFrame(columns=["pct_excluded","subj","method"])
tsnr_list=[]
source_oc_all=[]
source_tsnr_all=[]
...
df=pd.DataFrame(data=zdata_tsnr_masked, columns=["value","id","method"])
plotnine_obj=(ggplot(df,aes(x="method",y="value", fill="id"))+geom_boxplot(position=position_dodge(1)))
#plotnine_obj.draw()
plotnine_obj.save(filename="oc_tsnr.png",dpi=400)
